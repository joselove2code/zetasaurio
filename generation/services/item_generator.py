...
def select_random_feature(layer: Layer):
    random_percent = utils.generate_random_percent()
...
def apply_rule(rule: Rule, combination: Dict[str, Feature]):
    combination[rule.layer_to] = combination[rule.layer_to].layer.get_feature(
        rule.feature_to)
...
def generate_item_image(item: Item) -> Image:
    ...
    image.save(utils.item_image_path(item.id))
...
def store_provenance(provenances: dict):
    ...
    with open(utils.items_provenance_path(), "w") as provenance_file:
...
def generate_items(amount: int):
    ...
    dna_used = set()
    provenances = {}
    layers = layers_reader.read_layers()
    ...
    progress = utils.cyan(f'{amount}/{amount}')
    print(f'Generating {progress} items...' + utils.green('Done!'))
