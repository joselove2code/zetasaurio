...
class Layer:
    ...
    def __repr__(self) -> str:
        features_str_list = list(
            map(lambda feature: str(feature), self.features))
...
class Item:
    ...
    def generate_dna(self) -> str:
        filtered_features = [
            feature for feature in self.features if feature.layer.name not in config.omit_in_dna]
        feature_names = list(
            map(lambda feature: feature.name, filtered_features))
    ...
    def generate_metadata(self) -> str:
        attributes = list(
            map(lambda feature: feature.generate_metadata(), self.features))
        metadata = {
